...
**Cowork routines gotcha, same shape as the Cloudflare Worker one above:** the two routines that email the user's portfolio (`briefing-crypto-hebdo-cloud`, weekly, and `alerte-crypto-quotidienne-cloud`, every 4h) live entirely as Cowork trigger config (`update_trigger`/`list_triggers`), not as files in this repo — editing `data/portfolio.json` here changes what they read (both fetch it read-only via the `raw.githubusercontent.com` URL, same pattern as `aguilaradar-briefing-email-quotidien`), but their prompt text, cadence, and email design live outside this repo and won't show up in a diff or `git log`. A third, unrelated routine (`aguilaradar-briefing-email-quotidien`, daily) reads this repo's `digest.json`/`alerts.json`/`engine-history.json` for a pure market-analysis email — it carries no portfolio data and should stay that way.
